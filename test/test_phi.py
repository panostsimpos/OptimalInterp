...
def test_phi_1d_shape():
    mu = 1.0
    sigma = 2.0
    N_alpha_terms, max_beta_idx = 5, 7
    phi_g = oi.GaussianConvolutionPhi1D(max_beta_idx, mu, sigma)
    phi_wrapped = oi.WrappedGaussianConvolutionPhi1D(
        N_alpha_terms, max_beta_idx, 10, 0., 1., mu, sigma
    )
    N_beta_terms = 2*max_beta_idx + 1
...
def test_gaussian_phi_1d_eval():
    ...
    beta = jnp.arange(-max_beta_idx, max_beta_idx + 1)
...
def test_wrapped_gaussian_phi_1d_eval():
    mu = 1.0
    sigma = 2.0
    N_alpha, max_beta_idx, wrap_max_idx = 5, 7, 10
    phi = oi.WrappedGaussianConvolutionPhi1D(N_alpha, max_beta_idx, wrap_max_idx, 0., 1., mu, sigma)
    psi_t = jnp.linspace(-1, 1, N_alpha)
    Phi, _, _ = phi.evaluate(psi_t)
    ...
    # (k, alpha, beta)
    beta_psi = -jnp.arange(-max_beta_idx, max_beta_idx + 1).reshape(1,1,-1) * psi_t.reshape(1,-1,1)
...
def test_phi_1d_derivatives():
    ...
    N_alpha, max_beta_idx = 5, 7
    phi_g = oi.GaussianConvolutionPhi1D(max_beta_idx, mu, sigma)
    phi_wrapped = oi.WrappedGaussianConvolutionPhi1D(N_alpha, max_beta_idx, 10, 0., 1., mu, sigma)
    psi_t = jnp.linspace(-1, 1, N_alpha)
    for phi in [phi_g, phi_wrapped]:
        ...
        beta_s = jnp.arange(-max_beta_idx, max_beta_idx + 1)
