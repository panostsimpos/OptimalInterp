...
def test_gaussian_phi_1d_derivatives():
    ...
    second_deriv_0 = (
        char_funct_0(pts_pos_fd) - 2 * char_funct_0(pts) +
        char_funct_0(pts_neg_fd)
    ) / (FD_DELTA**2)
    ...
    pts = -beta_s * psi_t[-1]
    pts_pos_fd, pts_neg_fd = pts + FD_DELTA, pts - FD_DELTA
    def char_funct_1(t): return np.exp(1j * mu * t - 0.5 * sigma**2 * t**2)
    deriv_1 = (char_funct_1(pts_pos_fd) -
               char_funct_1(pts_neg_fd)) / (2 * FD_DELTA)
    assert np.array(Phi_prime[-1, :]) == pytest.approx(
        np.array(deriv_1), rel=10 * FD_DELTA
    )
    second_deriv_1 = (
        char_funct_1(pts_pos_fd) - 2 * char_funct_1(pts) +
        char_funct_1(pts_neg_fd)
    ) / (FD_DELTA**2)
