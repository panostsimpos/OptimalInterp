...
# %%
residual = oi.basis.create_collocated_basis_residual(
    t_grid, N_terms, psi, phi, fixed_orders, t_wts, jnp.real
)
...
hm = plt.imshow(
    velocity_eval[100:].T,
    extent=(eval_t_grid[100].item(), 1, x_min, x_max),
    aspect=1 / (x_max - x_min),
)
plt.plot(
    eval_t_grid,
    jnp.linspace(0, phi.mu, len(eval_t_grid)),
    color="k",
    linewidth=3,
    label=r"$\mu$",
)
