...
class SolutionApproximator(ABC):
    @abstractmethod
    def evaluate_all(self, params) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        pass


@jax.jit
def _linear_solution_evaluator(basis_eval: jnp.ndarray, basis_diff: jnp.ndarray, basis_diff2: jnp.ndarray, params: jnp.ndarray):
...
class LinearSolutionApproximator(SolutionApproximator):
    ...
    def __init__(self, basis: LinearBasis, evaluation_points: jnp.ndarray, N_terms: int):
    ...
    @chex.chexify
    def evaluate_all(self, params: jnp.ndarray):
